...
**BAD** RMSE 47 and r squared 0.26
...
Using only one feature, we got RMSE = 40. I dont know if we are going to the right direction or not. A lot of experimentation needed to find out. 
...
**Probably the best result so far**. RMSE = 38 and explains almost 48% of the data (r-sq)
...
```{r}
full_feat_prediction <- predict(full_model,  newdata = test_cars)
...
one_feat_prediction <- predict(one_feat_model, newdata =  test_cars)

postResample(pred = full_feat_prediction, obs = test_cars$price)
...
```{r}
tidy_test_cars <- test_cars %>% 
  mutate(full_sq_error = (price - full_feat_prediction)^2,
         one_feat_sq_error = (price - one_feat_prediction)^2,
         two_feat_sq_error = (price - two_feat_prediction)^2,
         two_feat_sq_error_2 = (price - two_feat_prediction_2)^2,
         two_feat_sq_error_3 = (price - two_feat_prediction_3)^2,
         two_feat_sq_error_4 = (price - two_feat_prediction_4)^2,
         two_feat_sq_error_5 = (price - two_feat_prediction_5)^2,
         two_feat_sq_error_6 = (price - two_feat_prediction_6)^2,
         three_feat_sq_error = (price - three_feat_prediction)^2,
         three_feat_sq_error_2 = (price - three_feat_prediction_2)^2,
         three_feat_sq_error_3 = (price - three_feat_prediction_3)^2,
         three_feat_sq_error_4 = (price - three_feat_prediction_4)^2) %>%
  pivot_longer(
    cols = 
      full_sq_error:three_feat_sq_error_4,
    names_to = "model_name",
    values_to = "sq_error")
...
Lets summarize the model and predictions 

```{r}
pred_summary <- tidy_test_cars %>% 
  group_by(model_name) %>%
  summarise(rmse = sqrt(mean(sq_error)))
pred_summary <- pred_summary %>% arrange(rmse)
pred_summary
...
From above summary , we can conclude that using three features : **horse power, engine size and highwy mpg** gives us the best model to predict the test data where we got the **lowest** rmse = 31.01. However, while noodling around the tuning grid, folds and features, sometimes we had different feeling that some other model works best. I hope if we tweak some more, for example with 10 folds, we might achieve even lower rmse. But one thing is for sure, we have to keep **noodling !**  
